...
namespace SignalRWebUI.Controllers
{
	public class DiscountController : Controller
	{
		...
		public async Task<IActionResult> CreateDiscount(CreateDiscountDto createDiscountDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7191/api/Discount", content);
		...
		public async Task<IActionResult> UpdateDiscount(UpdateDiscountDto updateDiscountDto)
		{
			...
			StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync($"https://localhost:7191/api/Discount/", content);
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage=await client.GetAsync($"https://localhost:7191/api/Discount/ChangeStatusToTrue/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
